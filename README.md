...
L'agent interprète les requêtes, résout les dépendances entre outils de manière autonome, et interagit avec Bitrix24 via un client API optimisé pour les gros volumes.
...
Développer un agent conversationnel connecté à Bitrix24, capable de gérer des tâches en langage naturel :
...
L'agent doit pouvoir créer, lister, modifier et supprimer des tâches, et retrouver un utilisateur par son prénom.
...
|---|---|---|---|---|
| **Llama 3.3 70B** (défaut) | 70B | Meta | Le plus capable en tool calling et raisonnement | Modèle principal — utilisé tant que le quota n'est pas atteint |
| **Qwen 3.6 27B** | 27B | Alibaba | Bon raisonnement, léger, rapide | Fallback quand Llama 3.3 atteint sa limite |
...
|---|---|---|
| **LLM / ChatModel** | Le cerveau qui génère du texte | `ChatGroq(model="llama-3.3-70b-versatile")` |
...
- **Sélecteur de modèle** : Changement de LLM en temps réel (Llama 3.3, Qwen 3.6, GPT-OSS).
